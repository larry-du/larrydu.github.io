...
function addTable ({row,col},{addTable,addCol}){
	console.log(row);
	content.insertAdjacentHTML('beforeend',`<div class="tableArea">${addTable()}</div>`);
	const totalRow =Array(Number(row)).fill(0);
	const totalCol =Array(Number(col)).fill(0);
	const tableFrom = document.querySelector(".tableFrom")
	tableFrom.insertAdjacentHTML('beforeend','<tbody class="tbody"></tbody>')
	const tbody = tableFrom.querySelector(".tbody");
